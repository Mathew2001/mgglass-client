    ...
    <div className="container" dir={dir} style={{marginTop: '100px'}}>
      ...
      <div className='row row-cols-md-2 row-cols-xl-3 g-4'>
        {mecklahonsByCatId && mecklahonsByCatId.map((mecklahon) => (
          <MeklahonCard key={mecklahon._id} name={mecklahon.name?.[language]} description={mecklahon.description?.[language]} imageGallery={mecklahon.imageGallery} id={mecklahon._id} link={`${ROUTES.MECKLAHONS}/${encodeURIComponent(mecklahonCat?.name?.[language])}/${encodeURIComponent(mecklahon.name?.[language])}`} type="array"/>
          ))}
        </div>
      </div>
    </>
